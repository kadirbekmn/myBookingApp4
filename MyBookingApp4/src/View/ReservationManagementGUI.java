...
public class ReservationManagementGUI extends JFrame {
    ...
    public ReservationManagementGUI() throws SQLException {
        ...
        reservationTable.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				try {
					reservation_id.setText(reservationTable.getValueAt(reservationTable.getSelectedRow(), 0).toString());
				} catch (Exception e4) {
    ...
    private void addReservation() throws SQLException {
        ...
        if (selectedEmployee != null && selectedCustomer != null && date != null && time != null) {
            ...
            } else if (!reservation.isEmployeeAvailable(reservation.getEmployeeId(), reservation.getReservationTime(), reservation.getEndTime())) {
    ...
    private void updateTables() throws SQLException {
        ...
        for (Operation op : operations) {
            operationModel.addRow(new Object[]{op.getOperationName(), op.getOperationTime(), op.getOperationPrice()});
        }
        ...
        reservationModel.setColumnIdentifiers(new Object[]{"Grup Id", "Çalışan Adı", "Müşteri Adı", "Operasyon Adı", "Toplam Operasyon Süresi", "Toplam Operasyon Ücreti", "Rezervasyon Saati", "Rezervasyon Bitiş Saati"});
        for (Reservation re : reservations) {
            String reservationTime = dateFormat.format(re.getReservationTime());
            String endTime = dateFormat.format(re.getEndTime());
            reservationModel.addRow(new Object[]{re.getGroupId(), re.getEmployeeName(), re.getCustomerName(), re.getOperationName(), re.getTotalOperationTime() + "dk", re.getTotalPrice() + "TL", reservationTime, endTime});
        }
